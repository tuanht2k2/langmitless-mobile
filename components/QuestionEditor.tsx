...
function QuestionEditor({ control, errors, onSuccess }: IProps) {
  ...
  const createPronunciationQuestion = useCallback(
    async (data: RequestInterfaces.IPronunciationRequest) => {
      try {
        const requestData = {
          topicId,
          content: data.content,
          audioSample: data.audioSample,
        };
        ...
        console.error("Lỗi khi tạo câu hỏi phát âm:", error);
        dispatch(overlayLoaded());
        throw error;
      }
    },
